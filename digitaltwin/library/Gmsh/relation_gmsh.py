...
class MeshingInput2OutputFile():
    def __init__(self, meshinginput, outputfile, properties=None):
        self.meshinginput = meshinginput
        self.outputfile = outputfile
        self.properties = properties if properties else {}
        self.type = "has_inputFile"
        
    def create_relationship(self, tx):
        if self.meshinginput.node is None or self.outputfile.node is None:
            raise ValueError("Both nodes must be created before creating a relationship.")
        # Construct a Cypher query to create a relationship with properties
        prop_string = ', '.join([f'{k}: ${k}' for k in self.properties])
        query = (
...
class MeshingOutput2OutputFile():
    def __init__(self, meshingoutput, outputfile, properties=None):
        self.meshingoutput = meshingoutput
        self.outputfile = outputfile
        self.properties = properties if properties else {}
        self.type = "has_outputFile"
        
    def create_relationship(self, tx):
        if self.meshingoutput.node is None or self.outputfile.node is None:
            raise ValueError("Both nodes must be created before creating a relationship.")
        # Construct a Cypher query to create a relationship with properties
        prop_string = ', '.join([f'{k}: ${k}' for k in self.properties])
        query = (
            f"MATCH (a:{self.meshingoutput.nodeType} {{{self.meshingoutput.identified_by}: $meshingoutput_name}}), "
            f"(b:{self.outputfile.nodeType} {{{self.outputfile.identified_by}: $outputfile_name}}) "
            f"CREATE (a)-[r:{self.type} {{{prop_string}}}]->(b) "
            f"RETURN r"
        )
