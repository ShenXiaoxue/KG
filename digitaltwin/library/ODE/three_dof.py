# -*- coding: utf-8 -*-
"""
Created on Mon Jan 22 16:18:15 2024
...
def Extract_ODE_input(parameter):
...
def simulator(N = 30, t_length=10, t_samps=1000,Ex='Hammer',Mi=M,Ki=K,Ci=C,Xi=[0,0,0],Vi=[0,0,0],disp=[0,0,0],d_init=[0,0]):
    ...
    if Ex=='Hammer':
        ...
        print("f_value", f.value(t=0.05))   #t=t)
    elif Ex=='White':
...
def fourier_transform(t,x,samps=2048):
    ...
    np.save(path+"Pxx_spec_dB", 10*np.log10(Pxx_spec))
    np.save(path+"freq", freq[i,0,:])
    return freq, Pxx_spec

# Defaults = (N=1,t_length=30,t_samps=int(30*8e4),Ex='Hammer',Mi=M,Ki=K,Ci=C,Xi=[0,0,0],Vi=[0,0,0],disp=[0,0,0],d_init=[0,0])
# disp=[0.5,10,0.1]


def ODE_input(parameters_):
    matrix_ = np.zeros(len(parameters_))
    for i in range(0, len(matrix_)):
        matrix_[i]= Extract_ODE_input(parameters_[i])
    return matrix_

    
def ODE_output(Mi=M, Ki=K, Ci=C):
    tt,xx = simulator(N=1,t_length=1,t_samps=int(1*2048),Ex='Hammer',Mi=M,Ki=K,Ci=C,Xi=[0,0,0],Vi=[0,0,0],disp=[0.0, 0.0, 0.0],d_init=[0,0])
    np.save(path+"tt", tt)
    np.save(path+"displacements", xx[:,1:4,:])
    return tt, xx[:,1:4,:]
...
#     tt,xx = simulator(N=1,t_length=1,t_samps=int(1*2048),Ex='Hammer',Mi=M,Ki=K,Ci=C,Xi=[0,0,0],Vi=[0,0,0],disp=[0.0, 0.0, 0.0],d_init=[0,0])  #disp=[0.05, 0.1, 0.1],
#     np.save(path+"tt", tt)
#     np.save(path+"displacements", xx[:,1:4,:])
#     fr,Pxx = fourier_transform(tt,xx)
#     fig, axs = plt.subplots(3, 2)
#     for i in range(0, 3):
#         axs[i, 0].plot(tt,xx[0, i+1, :])
#         axs[i, 0].set_xlim(0,1)
#         axs[i, 1].plot(fr[0, i, :], 10*np.log10(Pxx[0, i, :]))
#         axs[i, 1].set_xlim(1,30)
#         axs[i, 1].set_ylim(-120,-50)
#         axs[i, 1].set_xscale('linear')
        
#         # axs[i, 1].set_yscale('log')
#     # plt.plot(tt,xx[0, 3, :])
#     # plt.plot(tt,xx,fr,Pxx)
#     plt.show()
